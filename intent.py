...
def send_intent(intent_type, src_host, dst_host):
    """
    Function to send intent to Ryu controller via REST API.
    :param intent_type: Type of intent ('allow' or 'isolate')
    :param src_host: Source host (e.g., 'h1')
    :param dst_host: Destination host (e.g., 'h5')
    """
    url = "http://localhost:8080/intent/add"
    headers = {"Content-Type": "application/json"}

    # Resolve MAC addresses from host names
    src_mac = HOST_MAC_MAPPING.get(src_host)
    dst_mac = HOST_MAC_MAPPING.get(dst_host)
...
def main():
    ...
    while True:
        ...
        intent_choice = input("Enter choice (1 or 2): ").strip()
        ...
        send_intent(intent_type, src_host, dst_host)
        print("\n" + "-"*50 + "\n")
